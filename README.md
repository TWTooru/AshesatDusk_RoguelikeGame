# 回應市場需求的遊戲：《暮墓餘燼 (Ashes at Dusk)》

> **專案主題**：運用現代化開發工具與 AI 輔助協作，打造回應當前短影音快節奏、極簡視覺與即時回饋市場需求的超輕量 Roguelike 動作遊戲。
...
- **無縫實體房間機制 (Physical Seamless Rooms)**：清空波次後無需點選傳統繁瑣地圖，直接操控主角「走進左右牆壁」即可無縫切換房間，維持遊戲絕佳的流暢感。
- **雙重模式滿足多元需求**：
  - **正式模式（7 分鐘 / 7 關）**：適合完整深度體驗與構築樂趣。
  - **展示模式（1 分鐘 / 4 關）**：專為短影音展示、快節奏體驗與簡報試玩設計。
